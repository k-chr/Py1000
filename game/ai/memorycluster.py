...
class TakingTrickClusterMemory(Memory):
    ...
    def init_queues(self):
        self.cluster: Dict[int, Memory] = {i:Memory() for i in range(MIN_CARDS_IN_HAND, MAX_CARDS_IN_HAND + 1)}

    def remember_S_A_R_B(self, state: TakingTrickState, action: int, reward: float, behavior: float):
