...
def produce_sample(index: int) -> ndarray:
    empty: ndarray = zeros(MIN_SIZE_OF_BATCH if index < MIN_SIZE_OF_BATCH else index + 1)
    empty[index] = 1
    return empty
...
class Memory:
    ...
    def prepare_sample_batch(self, batch_size=MIN_SIZE_OF_BATCH, action_mapper: fun[[int], ndarray] =lambda x: produce_sample(x), 
        ...
        if(len(self.states_queue) < clipped):
            return None

        indices: List[int] = choice(range(len(self.states_queue)), clipped, replace=False)
