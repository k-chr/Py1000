...
class TakingTrickQPolicyDNNCluster(QPolicyNetwork):
    ...
    def __init__(self, fname: str, session: str, n_actions: int, batch_size: int, alpha: float, mem_dir: str, n_one_hot: int, 
                       start_from: datetime, flag: TrainingEnum, mode: NetworkMode):
        super().__init__(fname, session, n_actions, batch_size, alpha, mem_dir, n_one_hot, start_from, flag, mode)
        self.nodes : Dict[int, QPolicyNetwork] ={}

    def build_network(self):
        for i in range(2, MAX_CARDS_IN_HAND + 1):
            self.nodes[i] = QPolicyNetwork.get_instance(f'{self.network_name}_node_{i}', self.session, self.action_output_size, self.batch_size, 
    ...
    def get_instance(name: str, session: str, n_actions: int, batch_size: int, alpha: float, 
        ...
        l = [__instance for __instance in TakingTrickQPolicyDNNCluster.__instances if (
                ...
                and flag is __instance.flag and mode & __instance.mode and session == __instance.session
            )]
        __instance = None

        if not any(l):
            __instance = TakingTrickQPolicyDNNCluster(name, session, n_actions, batch_size, alpha, mem_dir, n_one_hot, start_from, flag, mode)
